...
function installLinux(linuxType) {
  return linuxFlavors.indexOf(linuxType) !== -1;
}
...
function favoritePlanet(currentPlanet) {
  ...
  var randoPlanet = planets[Math.floor(Math.random() * planets.length)];
...
Stapler.prototype.staplePapers = function (numPapers) {
  return (numPapers <= this.maxPapers);
};
...
Scientist.prototype.addDiscovery = function(discovery) {
  ...
  if (this.discoveries.length === 2) {
    result += this.discoveries[0] + " and " + this.discoveries[1] + ".";
  }
  if (this.discoveries.length === 1) {
    result += this.discoveries[0] + ".";
  }
  return result;
};
...
var christmasPresent = new Box('love', false);
...
var goodForYou = new Meal(['salad','salmon','rice']).containsJunkFood();
